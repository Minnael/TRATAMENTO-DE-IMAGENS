...
## <div align="center" styles="font-size:50px;" >Problema 01: Trabalhando com Fork()</div>
...
## <div align="center" styles="font-size:50px;" >Problema 02: Trabalhando com Threads</div>
Elabore um programa, escrito em C ou C++, em um sistema operacional com suporte a POSIX
(Linux, por exemplo), de tal modo que possam ser usados os compiladores GCC ou G++, para:
1) Criar um processo que lance uma thread mãe para fazer a abertura de uma imagem em
nível de cinza (que será disponibilizada e usada para detecção de bordas) e que
declare (globalmente) e crie as estruturas de dados necessárias para representar essa
imagem (como uma matriz I, por exemplo);
2) A thread mãe deverá criar duas threads filhas. Uma delas será responsável por detectar
as bordas da imagem na direção x, gerando a matriz global Rx, e a outra ficará
responsável pelas bordas na direção y, gerando a matriz global Ry. Para fins de
sincronização e, também para evitar o encerramento precoce das threads filhas, a
thread mãe deverá ficar suspensa através do comando join();
3) Por fim, após a execução das filhas, a thread mãe utilizará as informações das matrizes
Rx e Ry para gerar a matriz com as informações das bordas (matriz R). Esse resultado
deve ser salvo como uma imagem.
